...
echo "deb http://repo.mongodb.org/apt/ubuntu "$(lsb_release -sc)"/mongodb-org/3.2 multiverse" | sudo tee /etc/apt/sources.list.d/mongodb-org-3.2.list| sudo tee /etc/apt/sources.list.d/mongodb-org-3.2.list
...
sudo cp /home/ubuntu/environment/db/mongod.service /etc/mongod.service
